...
def showCatalog():
    ...
    latestItems = session.query(Item).order_by(desc(Item.date))
...
def showItemDetails(category_name, item_name):
    ...
    return render_template('itemDetails.html', categories=categories,
...
@app.route('/catalog/<string:category_name>/edit/<string:item_name>',
           methods=['GET', 'POST'])
...
@app.route('/catalog/<string:category_name>/delete/<string:item_name>',
           methods=['GET', 'POST'])
def deleteItem(category_name, item_name):
    ...
    if itemToDelete.user_id != login_session['user_id']:
        flash("You are not able to delete this item",
              "because it doesn't belong to you.")
        return redirect(url_for('showCatalog'))
    if request.method == 'POST':
...
def catalogJSON():
    ...
    for c in range(len(categories_list)):
        items = [i.serialize for i in session.query(Item).filter_by(
            category_id=categories_list[c]["id"]).all()]
        if items:
            categories_list[c]["Item"] = items
